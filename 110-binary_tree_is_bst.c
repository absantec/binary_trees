...
/**
 * check_sub_tree_Left - this check if all nodes are smaller than
 * the root specified
 * @node: the node in the tree to verify condition
 * @max: the value to be compared
...
/**
 * check_sub_tree_Right - the check if all the nodes are bigger than the
 * root specified
 * @node: the node in the tree to verify condition
 ...
 * @tree: the node that point to the tree to be checked
